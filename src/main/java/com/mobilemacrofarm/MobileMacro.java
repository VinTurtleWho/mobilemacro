                ...
                try {
                    ...
                    if (parts.length >= 2) {
                        ...
                        if (type.equals("record")) {
                            ...
                            FarmingMacro.getInstance().setPestOnly(false);
                            client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Mode set to NORMAL FARMING"));
                        } else if (type.equals("tool")) {
                            int slot = Integer.parseInt(parts[2]) - 1; // 1-9 to 0-8
                            FarmingMacro.getInstance().setToolSlot(slot);
                            client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Tool slot set to " + (slot+1)));
                        } else if (type.equals("vacuum")) {
                            int slot = Integer.parseInt(parts[2]) - 1;
                            FarmingMacro.getInstance().setVacuumSlot(slot);
                            client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Vacuum slot set to " + (slot+1)));
                        } else if (type.equals("end")) {
                            if (parts.length == 2) {
                                int px = client.player.getBlockX(); int py = client.player.getBlockY(); int pz = client.player.getBlockZ();
                            ...
                            if (type.equals("mode")) {
                                ...
                                FarmingMacro.getInstance().setYaw(Float.parseFloat(valStr));
                                client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Target Yaw set to " + valStr));
                            } else if (type.equals("pitch")) {
                                FarmingMacro.getInstance().setPitch(Float.parseFloat(valStr));
                                client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Target Pitch set to " + valStr));
                            }
                        }
                    }
                } catch (Exception e) {

...
public class FarmingMacro {
    ...
    private String targetMode = "pest"; // "pest" or "player"
    ...
    public void toggle(Minecraft client) {
        if (state == MacroState.IDLE) {
            if (pestOnlyMode) {
                state = MacroState.PEST_HUNTING; // Go straight to hunting for testing
                if (client.player != null) client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Started PEST ONLY Mode! Scanning for: " + targetMode));
            } else {
    ...
    // THE RADAR (Now supports Players for testing)
        ...
        for (net.minecraft.world.entity.Entity entity : client.level.entitiesForRendering()) {
            ...
            if (targetMode.equals("player")) {
                ...
                if (entity.hasCustomName()) {
                    ...
                    for (String p : PEST_NAMES) {
                        if (name.contains(p)) { isValid = true; break; }
                    }
                }
            }
    ...
    public void onTick(Minecraft client) {
        ...
        switch (state) {
            ...
            case PEST_FLY_UP:
                ...
                if (client.player.getY() > 130) {
                    InputController.setPressed(client.options.keyJump, false);
                    state = MacroState.PEST_HUNTING;
                    if (client.player != null) client.player.sendSystemMessage(Component.literal("§a[MobileMacro] Sky drop reached. Scanning for targets..."));
                }
                break;
            case PEST_HUNTING:
                ...
                if (currentTarget == null || !currentTarget.isAlive() || currentTarget.isRemoved() || currentTarget.distanceToSqr(client.player) > 400.0) {
                    ...
                    if (currentTarget == null) {
                        if (pestOnlyMode) {
                            // In test mode, just wait until someone walks by
                        } else {
            ...
            case ALIGNING:
                ...
                break;
            case FARMING:
                handleFarming(client); break;
            case SHIFTING_LANE:
                handleLaneShift(client); break;
            case RESTARTING:
                handleRestart(client); break;
            case PEST_ONLY_MODE:
                break; // Unused, we use PEST_HUNTING now
        }
    }
    ...
    private void handleFarming(Minecraft client) {
        ...
        InputControllecd ~/mobilemacro

cat << 'EOF' > src/main/java/com/mobilemacrofarm/FarmingMacro.java
package com.mobilemacrofarm;
import com.mobilemacrofarm.input.InputController;
import com.mobilemacrofarm.rotation.RotationHandler;
import com.mobilemacrofarm.state.MacroState;
...
public class FarmingMacro {
    ...
    private void safeClick(Minecraft client, AbstractContainerScreen<?> screen, Slot slot) {
        try {
            ...
            for (Object ec : clickTypeClass.getEnumConstants()) { if (ec.toString().equals("PICKUP")) { pickup = ec; break; } }
            for (Method m : client.gameMode.getClass().getMethods()) {
                if (m.getName().equals("handleInventoryMouseClick") && m.getParameterCount() == 5) {
